...
const elements = {
  ...
  navFormBtn: document.querySelector("#navFormBtn"),
  ...
  protectedSections: document.querySelector("#protectedSections")
};
...
async function fetchStore() {
  clearMessage("fetch");
  elements.protectedSections.hidden = true;
  ...
  try {
    ...
    storeAccess = body;
    elements.protectedSections.hidden = false;
...
async function submitForm(event) {
  ...
  try {
    ...
    if (!response.ok) {
      setMessage(body.message || "❌ Failed to save data. Please try again.", true, "form");
      return;
    }
    ...
    setMessage(error.message || "❌ Failed to save data. Please try again.", true, "form");
...
elements.modalOkBtn.addEventListener("click", (e) => {
    e.preventDefault();
    e.stopPropagation();
    resetAndReauth();
});

elements.navFormBtn.addEventListener("click", () => {
